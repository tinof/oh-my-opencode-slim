...
- Each service file exports a `RemoteMcpConfig` literal that points at the remote URL and optionally supplies API keys derived from the corresponding environment variable to avoid leaking secrets (`linkup.ts`, `context7.ts`, `grep-app.ts`).
