...
- Hooks are isolated (`hooks/auto-update-checker`, `post-edit-nudge`, `post-warpgrep-nudge`, `project-context`) and exported via `hooks/index.ts`, so the plugin simply registers them via the `event`, `experimental.chat.messages.transform`, and `tool.execute.after` hooks defined in `index.ts`.
...
- Plugin registration: `index.ts` registers agents, the tool map (background/task, `grep`, `ast_grep_*`, `lsp_*`), MCP definitions (`createBuiltinMcps`), and hooks (`createAutoUpdateCheckerHook`, `createPostEditNudgeHook`, `createPostWarpgrepNudgeHook`, `createProjectContextHook`); configuration hook merges those values back into the OpenCode config (default agent, permission rules parsed from `config/agent-mcps`, and MCP access policies).
...
- Hooks and helpers tie into external behavior: `hooks/auto-update-checker` reads `package.json` metadata, runs safe `bun install`, and posts toasts; `hooks/post-edit-nudge` nudges lsp_diagnostics after edits; `hooks/post-warpgrep-nudge` bridges WarpGrep to Serena structural tracing; `hooks/project-context` injects session-start context; `utils/logger.ts` centralizes structured logging used across modules.
